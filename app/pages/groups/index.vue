...
// ============================================================
// GROUPS INDEX — List, Create, and Join Groups
// ============================================================
// This page shows groups the user belongs to, with modals
// to create new groups or join via invite code.
//
// DATA FLOW (SpacetimeDB):
//   READS:  useTable(tables.Group)       → all groups (filtered client-side)
//           useTable(tables.GroupMember)  → user's memberships
//   WRITES: conn.reducers.createGroup({ name: '...' })
//           conn.reducers.joinGroup({ code: '...' })
//
...
//        allMembers.value.filter(m => m.profileId.toHexString() === conn.identity?.toHexString())
//      )
//      const myGroupIds = computed(() => new Set(myMemberships.value.map(m => m.groupId)))
//      const groups = computed(() =>
//        allGroups.value
//          .filter(g => myGroupIds.value.has(g.id))
//          .map(g => {
//            const membership = myMemberships.value.find(m => m.groupId === g.id)
//            return { ...g, role: membership?.role || 'MEMBER' }
//          })
//      )
//
//   3. Create group via reducer:
//      conn.reducers.createGroup({ name: newGroupName.value })
//      // The group will appear automatically via subscription — no need to refresh!
//
//   4. Join group via reducer:
//      conn.reducers.joinGroup({ code: joinCode.value })
//      // Same — subscription auto-updates the list
//
// REFERENCE: specs/spacetimedb-typescript.md § 4 (Reducers)
//            specs/erd.md (Group and GroupMember tables)
// ============================================================
...
// For now, using an empty array so the page renders
const groups = ref<{ id: string; name: string; code: string; role: string }[]>(
  [],
);
...
const createGroup = async () => {
  ...
  loading.value = true;
  try {
    // TODO: Replace with SpacetimeDB reducer call
    // conn.reducers.createGroup({ name: newGroupName.value })
    console.log(
      "TODO: conn.reducers.createGroup({ name:",
      newGroupName.value,
      "})",
    );
    showCreateModal.value = false;
    newGroupName.value = "";
    // No refresh needed — SpacetimeDB subscriptions auto-update!
  } catch (e: any) {
...
const joinGroup = async () => {
  ...
  try {
    // TODO: Replace with SpacetimeDB reducer call
    // conn.reducers.joinGroup({ code: joinCode.value })
    console.log("TODO: conn.reducers.joinGroup({ code:", joinCode.value, "})");
      ...
      <div class="flex justify-between items-center mb-8">
        <div>
          ...
          <p class="text-gray-400 mt-2">
            Manage your event lobbies and schedules
          </p>
        </div>
      ...
      >
        ...
        >
          ...
          <p class="text-gray-400 text-sm mb-6 line-clamp-2">
            Code:
            <span class="font-mono text-blue-300 bg-blue-500/10 px-1 rounded">{{
              group.code
            }}</span>
          </p>

          <div
            class="flex items-center justify-between text-sm text-gray-500 border-t border-gray-700 pt-4"
          >
        ...
        >
          ...
          <div class="flex justify-end gap-2">
            ...
            <button
              @click="createGroup"
              :disabled="loading"
              class="px-4 py-2 bg-blue-600 rounded-lg hover:bg-blue-500 disabled:opacity-50"
            >
              Create
            </button>
          </div>
        ...
        >
          ...
          <div class="flex justify-end gap-2">
            ...
            <button
              @click="joinGroup"
              :disabled="loading"
              class="px-4 py-2 bg-green-600 rounded-lg hover:bg-green-500 disabled:opacity-50"
            >
              Join
            </button>
          </div>
